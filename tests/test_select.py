...
def test_describe_dropped_is_empty_when_nothing_dropped() -> None:
    assert describe_dropped({}, 3) == ""


def test_describe_dropped_ranks_busiest_first() -> None:
    note = describe_dropped({"small/one": 1, "big/one": 9}, 3)
    assert note.startswith("10 collapsed by --per-repo 3: big/one (+9)")


def test_describe_dropped_truncates_long_lists() -> None:
