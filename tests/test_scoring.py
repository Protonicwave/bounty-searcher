...
def test_claimed_is_heavily_penalised():
...
def test_language_match_beats_mismatch():
...
def test_crowded_thread_is_demoted():
...
def test_spam_payout_is_flagged_and_scores_below_a_modest_real_one():
...
def test_absurd_payout_on_a_mid_size_repo_is_flagged():
    b = make(amount=40_000, stars=200)
    score_bounty(b, WEIGHTS)
    assert b.suspect


def test_forks_are_demoted():
...
def test_score_stays_in_range():
    for b in (make(amount=99_000, stars=0, comments=500, claimed=True), make(amount=5000)):
        assert 0.0 <= score_bounty(b, WEIGHTS) <= 100.0


def test_low_effort_labels_help():
